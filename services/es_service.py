...
class EsService:
    ...
    def get_hourly_task_stats(self, index, hours=24):
        """
        聚合每个 worker|marketplace 的 hourly stats，包括：
          - num_asins, successful_asins, failed_asins
          - avg_task_duration_ms
          - avg_spapi_duration_ms
          - avg_spapi_success_ms (只统计成功 API 的平均耗时)
        返回：
        {
            "worker|marketplace": [
                {
                    ...
                    "avg_spapi_duration_ms": 456,
                    "avg_spapi_success_ms": 400
                },
                ...
            ]
        }
                            ...
                            "aggs": {
                                "per_hour": {
                                    ...
                                    "aggs": {
                                        ...
                                        "task_count_sum": {"sum": {"field": "task_count"}},
                                        "task_duration_sum": {"sum": {"field": "task_duration_ms"}},
                                        "spapi_duration_sum": {"sum": {"field": "spapi_duration_ms"}},
                                        "spapi_success_duration_sum": {"sum": {"field": "spapi_success_duration_ms"}},
                                        "spapi_success_count_sum": {"sum": {"field": "spapi_success_count"}}
                                    }
                                }
                            }
                        }
        ...
        for worker_bucket in res["aggregations"]["by_worker"]["buckets"]:
            ...
            for mp_bucket in worker_bucket["by_marketplace"]["buckets"]:
                ...
                for hour_bucket in mp_bucket["per_hour"]["buckets"]:
                    task_count = int(hour_bucket["task_count_sum"]["value"])
                    spapi_success_count = int(hour_bucket["spapi_success_count_sum"]["value"])
                    ...
                    stats_list.append({
                        ...
                        "avg_task_duration_ms": (
                            int(hour_bucket["task_duration_sum"]["value"]) // task_count
                            if task_count else 0
                        ),
                        "avg_spapi_duration_ms": (
                            int(hour_bucket["spapi_duration_sum"]["value"]) // task_count
                            if task_count else 0
                        ),
                        "avg_spapi_success_ms": (
                            int(hour_bucket["spapi_success_duration_sum"]["value"]) // spapi_success_count
